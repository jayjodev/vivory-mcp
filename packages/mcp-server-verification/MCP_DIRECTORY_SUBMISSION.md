...
> **Current state (2026-06-05)**: `vivory-mcp-verification` **v0.14.0** — 13 tools / 4 categories (moat collapse). Install: `uvx vivory-mcp-verification`. v0.14.0 narrowed the agent surface 87→13 to the Vivory-only moat (law·reconcile·doi·provenance), dropping ~74 commodity public-API wrappers + surface-internal tools (repro/peer-review/forecast); web Tools keep breadth (parity decoupled to `web ⊇ MCP`). v0.13.0 had retired 2 forecast tools (`forecast_track_record`, `submit_forecast`) coupled to the retired `vivory.app/forecast` Hub sub-route (2026-06-04). v0.12.0 retired 10 low-uptake tools (4 Tier-U composers: `verify_claim`, `provenance_summary`, `policy_list_presets`, `policy_evaluate`; 6 parity-debt MCP-only with no web sibling: `verify_proxy_upgrade`, `verify_contract_admin_activity`, `verify_domain_owner_change`, `verify_domain_history`, `verify_place`, `place_search`). The sister `vivory-mcp-korea` and `vivory-mcp-kosis` packages are deprecated (v0.6.2 / v0.1.4 deprecation-only); Korean verdict tools (`kor_law_currency`, `kor_company_status`, `doi_retraction_status`, `kor_case_search`, `kor_bill_status`) live in this package. Bundle absorb 2026-06-01: Tools Pro $4.99/mo single key supersedes the prior $29/mo Vivory API Pro tier. The body of this document reflects the v0.5.1 submission packet (2026-05-20) and is kept for archival reference.
